...
class MobileUtil {
  constructor() {
    var UA = navigator.userAgent;
      ...
      this.isIos = /iphone|ipod|ipad/gi.test(UA) && !this.isAndroid; // 据说某些国产机的UA会同时包含 android iphone 字符
      this.isMobile = this.isAndroid || this.isIos;  // 粗略的判断
  }
  ...
  fixScreen(win, doc){
    ...
    if ( !metaEl ) { // REM
        ...
        metaEl.content = this.fillScale(scale);
        ...
        refreshRem();
    } else if ( this.isMobile && !matchScale && ( matchWidth && matchWidth[1] != 'device-width' ) ) { // 定宽
        ...
        if ( scale < 1 ) {
            metaEl.content = metaCtt + ',' + this.fillScale(scale);
        }
    }
  }
