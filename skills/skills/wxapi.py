...
def activities_myunpay_list(req):
	...
	_sql = "select distinct b.title,b.id,b.price_child,b.act_id,a.id,DATE_FORMAT(a.createtime,'%%Y-%%m-%%d'),b.sign_type,a.part_group,a.part_profession,b2.price_child from 6s_signup a left join 6s_activity b on a.act_id=b.id left join 6s_preinfo b2 on b.preinfo_id=b2.id left join 6s_user e on a.user_id=e.id where ( (b.price_child>0 or b2.price_child>0) and not exists (select c.id from 6s_prepay_info c left join 6s_wxpay d on c.out_trade_no=d.out_trade_no and c.openid=d.openid where b.id=c.act_id and c.status=1 and d.status=1) ) and e.openid='%s' and e.openid=c.openid and a.status=1 and b.status=1 and (b2.id is null or b2.status=1) and a.status=1 and b.status=1 and e.status=1 order by a.createtime desc limit %d offset %d;"%(openid,pagesize,pagesize*(page-1))
	count,rets=dbmgr.db_exec(_sql)
	if count >= 0:
		for i in range(count):
