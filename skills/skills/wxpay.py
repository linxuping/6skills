...
class WxPayConf_pub(object):  
    ...
    NOTIFY_URL = "http://www.6skills.com/wxpay"  
...
class UnifiedOrder_pub(Wxpay_client_pub):  
    ...
    def getPrepayInfo(self):  
        self.postXml()  
        return self.xmlToArray(self.response)  
...
class DownloadBill_pub(Wxpay_client_pub):  
    ...
    def createXml(self):  
        """生成接口参数xml"""  
        if any(self.parameters[key] is None for n ("bill_date", )):  
            raise ValueError("missing parameter")  
...
class NativeCall_pub(Wxpay_server_pub):  
    ...
    def getProductId(self):  
        """获取product_id"""  
        product_id = self.data["product_idnd  
...
class NativeLink_pub(Common_util_pub):  
    ...
    def createLink(self):  
        ...
        self.parameters["nonce_str"] = self.createNoncestr()  #随机字符串  
        self.parameters["sign"] = self.getSign(self.parameters)  #签名            
        bizString = self.fozQueryParaMap(self.parameters, false)  
...
def get_prepay_info(out_trade_no, body, total_fee, notify_url, trade_type, openid):
	...
	pub.setParameter("body", body)
	pub.setParameter("total_fee", total_fee)
